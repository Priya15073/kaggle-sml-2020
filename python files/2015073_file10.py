...
test_label="/Users/priyarajpurohit/Desktop/Desktop – Priya’s MacBook Pro/bonus-sml-2020/2015073_model10.csv"
model_label="2015073_model10.pkl"
X_train_raw = []
HOG_x_train=[]
HOG_x_test=[]
y_train_raw = []
...
for i in range(16000):
    ...
    #print(cv2.imread(f))
    im1 = cv2.imread(f)
    fd = hog(im1, orientations=8, pixels_per_cell=(16,16),cells_per_block=(1, 1),block_norm= 'L2', multichannel=True)
    HOG_x_train.append(fd)
...
pca.fit(HOG_x_train)
HOG_x_train = pca.transform(HOG_x_train)
HOG_x_test = pca.transform(HOG_x_test)
...
#     d[i]=weights[i]
# print(d)
X_train1, X_val, y_train1, y_val = train_test_split(HOG_x_train, y_train_raw, test_size=0.2, random_state=1)
print("yay")
...
if(accuracy>0.26):
    ...
    if s>0.28:
        ...
        predicted= rf_class.predict(HOG_x_test)

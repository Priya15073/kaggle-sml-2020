...
test_label="/Users/priyarajpurohit/Desktop/Desktop – Priya’s MacBook Pro/bonus-sml-2020/2015073_model18.csv"
model_label="2015073_model18.pkl"
hog_train = []
hsv_train=[]
...
for i in range(16000):
    ...
    fd = hog(im, orientations=15, pixels_per_cell=(16, 16),cells_per_block=(2, 2),block_norm= 'L2')
    hog_train.append(fd)
...
for train_index, test_index in sss.split(X_train_raw, y_train_raw):
    X_train1, X_val = X_train_raw[train_index], X_train_raw[test_index]
    y_train1, y_val = y_train_raw[train_index], y_train_raw[test_index]

#X_train1, X_val, y_train1, y_val = train_test_split(X_train_raw, y_train_raw, test_size=0.2, random_state=1)
print("yay")


clf= RandomForestClassifier(n_estimators=600,max_depth=30, random_state=42,class_weight='balanced',max_features='sqrt')

#rf_class= RandomForestClassifier(n_estimators=200,max_depth=30, random_state=0,class_weight='balanced')
#clf=LDA(solver='svd',n_components=23)
